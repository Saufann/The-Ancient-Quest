...
class StoryManager {
    ...
    private void initStory() {
        ...
        scene5.addChoice("🎮 Continue Adventure [Coming Soon]", 0);
        scenes.add(scene5);
    ...
    void handleClick(int x, int y) {
        ...
        if (currentScene.hasBattle && currentScene.choices.isEmpty()) {
            gamePanel.startBattle(currentScene.battleEnemy);
            return;
        }
    ...
    private void drawBackground(Graphics2D g, String bg) {
        if (bg.equals("Forest")) {
            GradientPaint sky = new GradientPaint(0, 0, new Color(50, 100, 150), 0, 400, new Color(100, 150, 100));
            g.setPaint(sky);
            g.fillRect(0, 0, 1000, 700);
            
            g.setColor(new Color(34, 139, 34, 180));
            for (int i = 0; i < 12; i++) {
                int height = 100 + (int)(Math.random() * 100);
                g.fillOval(i * 100 - 30, 350 - height, 120, height);
            }
            
            g.setColor(new Color(139, 69, 19));
            for (int i = 0; i < 12; i++) {
                int height = 100 + (int)(Math.random() * 100);
                g.fillRect(i * 100 + 25, 350 - height, 20, height);
            }
        } else if (bg.equals("Ruins")) {
            GradientPaint sky = new GradientPaint(0, 0, new Color(60, 60, 80), 0, 400, new Color(80, 70, 90));
            g.setPaint(sky);
            g.fillRect(0, 0, 1000, 700);
            
            g.setColor(new Color(70, 70, 70));
            g.fillRect(100, 180, 250, 270);
            g.fillRect(650, 180, 250, 270);
            g.setColor(new Color(50, 50, 50));
            g.fillRect(120, 200, 80, 120);
            g.fillRect(670, 200, 80, 120);
        }
    }
